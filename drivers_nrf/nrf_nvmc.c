...
void nrf_nvmc_read_byte(uint32_t address, uint8_t* value)
{
	*value = *(uint8_t*)address;
}


void nrf_nvmc_read_bytes(uint32_t address, uint8_t * src, uint32_t num_bytes)
{
	uint32_t i;
	for(i=0;i<num_bytes;i++)
	{
	 nrf_nvmc_read_byte(address+i,&src[i]);
	}
}
